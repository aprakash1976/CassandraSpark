...
object MatchMaker {
  ...
  def main (args: Array[String]) {
    ...
    val counts = paraRdd.flatMap(p => p.split(" ")).map(region => (region, 1)).reduceByKey(_ + _)

    counts.collect().foreach {
      case (region, count) => println(region + ":" + count)
    }

    
    //val counts = matchmakerRDD.count
    
    println("number of records in MATCH_MAKER_SC_RAW_DATA - " + counts)
